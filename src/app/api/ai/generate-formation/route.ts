...
// Plan de démo : sert aussi de filet de sécurité si le gateway échoue.
function demoPlan(t: string) {
  return {
    titre: t,
    objectifs: [
...
export async function POST(req: Request) {
  ...
  if (!aiEnabled()) {
    return Response.json(demoPlan(t));
  }
  ...
  try {
    ...
    console.error("[ai] generate-formation, repli démo :", e instanceof Error ? e.message : e);
    return Response.json(demoPlan(t));
  }
}
