...
function demoAnswer(message: unknown, context: unknown) {
  return `Bonne question ! Voici une explication claire.
...
export async function POST(req: Request) {
  ...
  if (!aiEnabled()) {
    return new Response(streamDemoText(demoAnswer(message, context)), {
      headers: { "Content-Type": "text/plain; charset=utf-8" },
    });
  }

  // Mode IA réel (AI Gateway), avec repli démo si l'appel échoue.
  try {
    ...
    console.error("[ai] chat, repli démo :", e instanceof Error ? e.message : e);
    return new Response(streamDemoText(demoAnswer(message, context)), {
      headers: { "Content-Type": "text/plain; charset=utf-8" },
    });
  }
