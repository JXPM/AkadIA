...
// Heuristique de démo : score basé sur la richesse du prompt. Sert aussi de
// filet de sécurité si l'appel au gateway échoue (clé invalide, quota…).
function demoScore(text: string) {
...
export async function POST(req: Request) {
  ...
  if (!aiEnabled()) {
    return Response.json(demoScore(text));
  }
  ...
  try {
    ...
    console.error("[ai] prompt-score, repli démo :", e instanceof Error ? e.message : e);
    return Response.json(demoScore(text));
  }
}
